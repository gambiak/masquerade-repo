...
export default async function Admin(){
 const supabase=await createClient();const {data:{user}}=await supabase.auth.getUser();if(!user||user.email!==process.env.ADMIN_EMAIL)redirect("/");
 const {data:puzzles}=await supabase.from("puzzles_private").select("id,clue_text,difficulty_band,status,difficulty_score").order("created_at",{ascending:false}).limit(50);
