...
export async function POST(req:Request){
 ...
 const c=await getPool().connect(); try{
   ...
   if(existing?.status==='quit'){
     ...
     await c.query(`delete from puzzle_results where session_id=$1`,[existing.id]);
     const r=await c.query(`update game_sessions set status='active',current_position=1,started_at=now(),completed_at=null,score=0,hints_used=0,pure_solves=0,solve_time_ms=0 where id=$1 returning *`,[existing.id]);
     await c.query('commit'); return NextResponse.json({session:r.rows[0]});
   }
   const r=await c.query(`insert into game_sessions(user_id,daily_game_id,difficulty_band) values($1,$2,$3) returning *`,[user.id,game.id,difficulty]);
   await c.query('commit'); return NextResponse.json({session:r.rows[0]});
